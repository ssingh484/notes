...
Also ran rpcclient and dumped users using the enumdomusers command which did have an extra user in there for some reason:
...
santi
```
Did an AS-REP roast against this user list and got a hash:
...
Used john against this hash to get the password:

```
svc-alfresco : s3rvice
```

Ran bloodhound ( Figure out the CE edition shennanigans) and found out I can add myself to a group to give me WriteDacl which let's me give myself permissions do a DCSync attack.

This led to dumping the hashes using impacket-secretsdump:
...
Used evil-winrm to login by passing the hash at this point:

```
447fd5c76223d1858b302ceaac6a78d9
```
